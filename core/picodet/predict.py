...
class PicoDet:
    def __init__(self, det_model_path):
        ...
        :param det_model_path: 物体检测模型的路径。
        """
        self.pred_config = PredictConfig(det_model_path)  # 假设PredictConfig为外部库或自定义类
        self.detector = Detector(self.pred_config, det_model_path)  # 假设Detector为外部库或自定义类
    ...
    def detect(self, img, threshold = 0.6):
        ...
        if im_bboxes_num > 0:
            bbox_results = result['boxes'][0:im_bboxes_num, 2:]
            id_results = result['boxes'][0:im_bboxes_num, 0]
            score_results = result['boxes'][0:im_bboxes_num, 1]

            for idx in range(im_bboxes_num):
                if float(score_results[idx]) >= threshold:
                    detection_results["result"].append({
                            "type": self.pred_config.labels[int(id_results[idx])],
        ...
        print(detection_results["result"])
        return detection_results["result"]
    ...
    def predict_image(self, img):
        ...
        im, im_info = preprocess(im_path, self.detector.preprocess_ops)
...
if __name__ == '__main__':
    ...
    model = PicoDet(det_model_path)
    result = model.detect(img,threshold)
