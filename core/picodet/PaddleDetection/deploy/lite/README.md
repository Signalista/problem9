...
python python/lite/convert_yml_to_json.py output_inference/picodet_s_320_coco/infer_cfg.yml
```
...
```shell
mkdir python
cp main *runtime_config.json python/
cd python
mkdir model_det
...
adb push python /data/local/tmp/

adb shell
cd /data/local/tmp/python
export LD_LIBRARY_PATH=/data/local/tmp/python:$LD_LIBRARY_PATH
